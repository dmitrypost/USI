<?php
	...
	//only checks to see if data is valid
	function ParticipantDataValid($firstName,$lastName,$email,$role,$projectId)
	{
		//variables length test
		if (!(strlen($firstName) > 0 || strlen($lastName) > 0 || strlen($email) > 0 || strlen($role) > 0 || $projectId != 0))
		{ 
			echo "length test failed";
			return FALSE;
		}
		else
		{
	...
	function FormattedProjectAlterProcessingButton($AddEdit)
	{
		if ($AddEdit)
		{
	...
	function AddRegisteredParticipant($email,$role,$projectId)
	{
		$con = Open();
	...
	//adds the new user and sends them the email about being added as a user
	function AddUnregisteredParticipant($firstName,$lastName,$email,$role,$projectId)
	{
